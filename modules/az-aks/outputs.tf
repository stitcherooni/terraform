...
output "host" {
  ...
  value       = { for aks in azurerm_kubernetes_cluster.this : aks.name => aks.kube_admin_config.0.host }
  sensitive   = true
}
output "client_certificate" {
  description = "public certificate used by clients to authenticate to the Kubernetes cluster."
  value       = { for aks in azurerm_kubernetes_cluster.this : aks.name => aks.kube_admin_config.0.client_certificate }
  sensitive   = true
}
output "client_key" {
  description = "private key used by clients to authenticate to the Kubernetes cluster"
  value       = { for aks in azurerm_kubernetes_cluster.this : aks.name => aks.kube_admin_config.0.client_key }
  sensitive   = true
}
output "cluster_ca_certificate" {
  description = "public CA certificate used as the root of trust for the Kubernetes cluster."
  value       = { for aks in azurerm_kubernetes_cluster.this : aks.name => aks.kube_admin_config.0.cluster_ca_certificate }
  sensitive   = true
}
